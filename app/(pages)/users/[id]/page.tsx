...
// 🚀 AMÉLIORATION: Fonction de récupération de données côté serveur
export async function loadData({ params }: { params: { id: string } }) {
  console.log(`[SSR] 📊 Loading user data for ID: ${params.id}`);

  try {
    // En production, cela ferait un appel à votre API ou DB
    // Pour la démo, on simule avec des données mockées
    ...
    const user = mockUsers[params.id] || null;
    ...
    console.error(`[SSR] Error loading user ${params.id}:`, error);
